...
export const supabaseBackend = {
  ...
  /**
   * Actualiza la calificación del negocio (JSONB en branding)
   */
  updateBusinessRating: async (rating: import('../types').BusinessRating): Promise<import('../types').Business> => {
    const { data: userData } = await supabase.auth.getUser();
    const userId = userData.user?.id;
    if (!userId) throw new Error('Usuario no autenticado');
    const cached = businessCacheByUser.get(userId);
    ...
    // Merge con la nueva configuración de rating
    const updatedBranding = {
      ...currentBranding,
      rating
    };
