...
export const FrequentClientsList: React.FC<FrequentClientsListProps> = React.memo(({ clients }) => {
  if (clients.length === 0) {
    return (
    ...
    <div className="bg-surface rounded-lg shadow-md border border-default p-4 h-full">
      ...
      <div className="space-y-3">
        {clients.map((client, index) => (
          <div key={index} className="flex items-center justify-between p-3 bg-gray-50 dark:bg-gray-800/50 rounded-lg border border-gray-100 dark:border-gray-700 hover:border-primary/30 transition-colors">
            <div className="flex items-center">
              ...
              <div>
                <p className="font-medium text-gray-800 dark:text-gray-200 text-sm">{client.cliente}</p>
                <p className="text-[10px] text-gray-500 dark:text-gray-400">Última visita: {new Date(client.ultima_visita).toLocaleDateString()}</p>
              </div>
            </div>
