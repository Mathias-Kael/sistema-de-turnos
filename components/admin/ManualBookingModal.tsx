        ...
        try {
            ...
            if (selectedEmployeeId === 'any') {
                ...
                if (availableEmployee) {
                    ...
                    alert(`No se encontró un ${business.branding?.terminology?.labels?.singular || 'empleado'} disponible para este horario.`);
                    return;
                }
            }
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (selectedEmployeeId === 'any') {
            ...
            if (availableEmployee) {
                ...
                alert(`No se encontró un ${business.branding?.terminology?.labels?.singular || 'empleado'} disponible para este horario. Por favor, verifica los horarios del personal.`);
                return;
            }
        }
            ...
            <form onSubmit={handleSubmit} className="bg-surface rounded-lg shadow-2xl p-6 md:p-8 max-w-2xl w-full max-h-[calc(100vh-2rem)] overflow-y-auto text-primary focus:outline-none" role="dialog" aria-modal="true" onClick={e => e.stopPropagation()}>
                ...
                <div className="space-y-4">
                    ...
                    {selectedServices.length > 0 && (
                        <fieldset className="border border-default p-4 rounded-md bg-surface">
                             <legend className="font-semibold px-2 text-primary">{business.branding?.terminology?.labels?.singular || 'Empleado'}</legend>
                             <select value={selectedEmployeeId || ''} onChange={e => setSelectedEmployeeId(e.target.value)} required className="p-2 w-full border border-default rounded-md bg-surface text-primary" disabled={eligibleEmployees.length === 0}>
                                <option value="" disabled>Seleccionar {business.branding?.terminology?.labels?.singular || 'empleado'}</option>
                                {eligibleEmployees.length > 0 ? (
                                    ...
                                    <option disabled>No hay {business.branding?.terminology?.labels?.plural || 'empleados'} elegibles para esta selección</option>
                                )}
                            </select>
                        </fieldset>
                    )}
